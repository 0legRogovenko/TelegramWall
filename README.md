...
- **Доставка** — каждый пост приходит отдельным сообщением: название канала —
  ссылка на оригинал, под постом кнопка «📝 Саммари». Альбом — одно сообщение.
  При добавлении канала приходят его последние посты; отправку больших пачек
  сглаживает рейт-лимитер.
- **Медиа доставляются** — фото, видео, аудио и документы до `MEDIA_MAX_MB`
  ...
  на оригинал. В режиме авто-саммари медиа тоже сохраняется — саммари
  приходит подписью к нему.
